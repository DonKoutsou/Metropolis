class_name MySafeResourceLoader
...
static func load(path:String) -> Resource:
	...
	var file_as_text = file.to_string()
	##file.close()

	# Use a regex to find any instance of an embedded GDScript resource.
	var regex:RegEx = RegEx.new()
	var _t = regex.compile("type\\s*=\\s*\"GDScript\"\\s*")	
	...
	var _t2 = extResourceRegex.compile("\\[\\s*ext_resource\\s*.*?path\\s*=\\s*\"([^\"]*)\".*?\\]")
